...
def inject_custom_css():
    """Inject CSS custom untuk tema yang lebih modern"""
    st.markdown("""
    <style>
    /* Background utama */
    .main {
    ...
    [data-testid="stSidebar"] {
        background: linear-gradient(195deg, #2d3748 0%, #4a5568 100%);
        color: white;
    }
    ...
    /* Button secondary */
    div[data-testid="column"] .stButton > button[type="secondary"] {
        background: linear-gradient(90deg, #f44336 0%, #d32f2f 100%);
    }
    
    div[data-testid="column"] .stButton > button[type="secondary"]:hover {
        background: linear-gradient(90deg, #e53935 0%, #c62828 100%);
    }
    
    /* Button primary */
    div[data-testid="column"] .stButton > button[type="primary"] {
        background: linear-gradient(90deg, #2196F3 0%, #1976D2 100%);
    }
    
    div[data-testid="column"] .stButton > button[type="primary"]:hover {
    ...
    .stTextInput input:focus, .stNumberInput input:focus, .stSelectbox select:focus, .stTextArea textarea:focus, .stDateInput input:focus {
    ...
    div[data-testid="stAlert"] div[role="alert"][style*="background-color: rgb(253, 237, 237)"] {
            ...
            if submitted:
                if not username or not password:
                    ...
                    if success:
                        ...
                        st.session_state.notification = f"Login berhasil! Selamat datang {username}"
...
def show_input_page():
    ...
    if submitted:
        ...
        if success:
            st.success(message)
            st.session_state.notification = f"Data '{id_batch}' berhasil disimpan!"
            st.session_state.notification_type = "success"
        ...
        if st.session_state.get('edit_mode', False) and st.session_state.get('edit_id') == id_batch:
            ...
            with st.form(f"form_edit_{id_batch}"):
                ...
                if submit_edit:
                    ...
                    if success_update:
                        st.success(msg)
                        st.session_state.notification = f"Data '{id_batch}' berhasil diedit!"
                        st.session_state.notification_type = "success"
        ...
        if st.session_state.get('delete_confirm', False) and st.session_state.get('delete_id') == id_batch:
            ...
            with col_yes:
                if st.button(f"✅ YA, HAPUS {id_batch}", key=f"confirm_yes_{id_batch}", use_container_width=True):
                    ...
                    if success_delete:
                        st.success(msg)
                        st.session_state.notification = f"Data '{id_batch}' berhasil dihapus!"
                        st.session_state.notification_type = "success"
        ...
        with col_delete1:
            ...
            if st.button("🗑️ HAPUS DATA INI", use_container_width=True, type="secondary"):
                if delete_id:
                    ...
                    if success:
                        st.success(msg)
                        st.session_state.notification = f"Data '{delete_id}' berhasil dihapus!"
                        st.session_state.notification_type = "success"
                        time.sleep(0.5)
                        st.rerun()
                    else:
            ...
            if st.session_state.get('show_delete_all', False):
                ...
                if confirm:
                    if st.button("✅ KONFIRMASI HAPUS SEMUA", use_container_width=True, type="primary"):
                        ...
                        if success:
                            st.success(msg)
                            st.session_state.notification = "Semua data berhasil dihapus!"
                            st.session_state.notification_type = "success"
            ...
            with st.form("add_user_form"):
                ...
                if st.form_submit_button("➕ TAMBAH USER"):
                    if new_username and new_password:
                        ...
                        if success:
                            st.success(msg)
                            st.session_state.notification = f"User '{new_username}' berhasil ditambahkan!"
                            st.session_state.notification_type = "success"
                            time.sleep(0.5)
                            st.rerun()
                        else:
        ...
        with col_backup1:
            ...
            if st.button("🔄 BUAT BACKUP SEKARANG", use_container_width=True):
                ...
                if success:
                    ...
                    st.session_state.notification = "Backup berhasil dibuat!"
                    st.session_state.notification_type = "success"
                    time.sleep(0.5)
                    st.rerun()
                else:
        ...
        with col_backup2:
            ...
            if st.button("🗜️ BUAT ZIP BACKUP", use_container_width=True):
                ...
                if success:
                    ...
                    st.session_state.notification = "Zip backup berhasil dibuat!"
                    st.session_state.notification_type = "success"
